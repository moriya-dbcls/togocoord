...
**決定（2026-09-25）: 1つの位置に対する複数の対応は返さない。** 変換先の配列ごとに最良の1つを答えとし、UCSC liftOver の `-multiple` に相当するものは設けない（liftOver も既定は同じく1つ。Ensembl の assembly converter は該当する断片をすべて返すが、そちらには倣わない）。重複領域の位置も1箇所に変換され、利用者に選択を求めない。
