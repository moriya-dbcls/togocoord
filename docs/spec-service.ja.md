...
**1つの位置が複数箇所に写る場合（2026-09-25）**: ゲノムのアライメントでは、重複領域・反復配列・パラログのために、1つの元位置が複数のチェーンに覆われることがある。チェーンの取り込みは1チェーンを1つの edge としてすべて残す（spec-ingest §14。source 側で一対一に絞るのは PAF の取り込みだけ）ので、保存先には全部あるが、「各配列には1回だけ到達する」が出力を決める。

- **別の配列**（別の染色体、alt scaffold）への対応は、それぞれ別の結果として返る。
- **同じ配列**の別の位置への対応は1つに畳まれ、他があったことは応答に出ない。
- `liftover` の辺はすべて同じコストなので、2本のチェーンを通る経路は (コスト, 迂回数, 経路長) で同点になる。勝つのは先にキューに入った方で、それは保存先の edge の並び＝チェーンファイルの並びに従う。UCSC はチェーンを score 降順に並べているので、たいていは最良のチェーンが勝つが、これは偶然であり、edge に記録している `score` はコストに使っていない。

GRCh37 → GRCh38 での実測: アライメントが覆う位置のうち 1.7% が複数のチェーンの下にある（chunk の外接範囲での判定なので上限寄り）。標本19例のうち15例は変換先が1つの配列で、畳まれる側にあたる。無作為な1,500件の位置変換では、返ってきた結果は常に1件だった。

**決定（2026-09-25）: 1つの位置に対する複数の対応は返さない。** 変換先の配列ごとに最良の1つを答えとし、UCSC liftOver の `-multiple` に相当するものは設けない（liftOver も既定は同じく1つ。Ensembl の assembly converter は該当する断片をすべて返すが、そちらには倣わない）。重複領域の位置も1箇所に変換され、利用者に選択を求めない。ただし*別の配列*への対応はそれぞれ別の結果として返り、その中の `results[0]` が syntenic な側である保証はない（§4）。
...
2. **同じ種のアセンブリは、経路が必要とすれば、指定がなくても種ごとに2回までまたぐ。** アライメントは注釈のあるアセンブリを中心に用意するので（spec-ingest §20）、中心でない2つの間は中心を経由する（ゼニゴケ cmMarPoly1.2 → MpTak_v7.1 → v3.1）。種をまたいだ先でも、その種のアセンブリを同じだけまたげる（mm10 → mm39 → hg38 → hg19）。 例: GRCh37 の位置 → chain → GRCh38 → CDS → タンパク質。経路には `liftover` の段が残るので、どこでアセンブリを移ったかが分かる。
...
- 変換先の種類（category）
- コスト
- `coverage`: この結果が入力のどれだけを運んでいるか（0〜1。下記）
- `approximate`
- 向き（orientation）
- 経路（`path`）

**結果は部分的でありうる（2026-09-25）。** 各段はブロックが覆う位置しか写さないので、結果が入力の一部しか運ばないことがある。ゲノムの区間を転写産物に変換すればエキソンの分だけが残り、別の種へ移せばアライメントが覆う分だけが残る。`coverage` はその割合を返す（残基はコドンの3塩基として数えるので、残基とそのコドンは 1）。`/v1/convert` の `minMatch` は、その割合に満たない結果を落とす。UCSC liftOver の `-minMatch` と同じ考えだが、既定値は設けない。ここでは部分的な結果は失敗ではなく通常のことだから。

同コストの結果は、まず優先タグ、次に `coverage` の順に並べる。**`results[0]` は最も安いというだけで、正しいとは限らない**。1つのアライメントの2本のチェーンはコストで分かれず、チェーンの score も coverage も、どちらの座位が syntenic かを語らないため。

きっかけになった実例: BRCA1 から 45kb 離れた 621bp の CRE `fanta:FCHS_174178` をマウスへ変換すると、結果が2件返る。マウス chr8 が入力の 20.8%（chain score 21,959）、syntenic な Brca1 座位のマウス chr11 が 17.9%（chain score 3,323）。先頭を採った利用側が、CRE をパラログ領域に結び付けてしまった。chain score で並べても coverage で並べても chr11 は選ばれない。確かなのは、どちらも 621bp の断片でしかないことで、`minMatch=0.95` ではどちらも返らない。これは liftOver の既定の挙動でもある。BRCA1 周辺の CRE 76件のうち、64件は期待どおりマウス chr11 に落ち、変換できるのは 66件、`minMatch=0.95` で残るのは 24件。
...
`service/public/`（ビルド不要の HTML・JavaScript・CSS と favicon。コンセプト版の配色とフォントを引き継ぐ）。REST API だけを使う薄いクライアントで、状態は URL（`?loc=&to=&db=&taxon=&assembly=&codon=`）に持つので、画面の URL をそのまま共有できる。

- Location ID を入力して、変換先（直接つながる配列、ゲノム、遺伝子領域、転写産物、タンパク質、構造）、データベース（既定は「any database」。結果の絞り込み）、生物種（既定は「same species」。読み込んだ生物種から選ぶ）を選ぶ。アセンブリの選択は、1つの種に複数のアセンブリを読み込んだときだけ表示する（既定は「default assembly」。選択肢は `GRCh37.p13 / hg19` のように UCSC 名も示し、生物種、accession、アノテーションの有無はマウスを重ねると表示する）。入力のカードには、生物種、アセンブリ、アセンブリの配列名で書かれていればその名前（`written as chr7`）を表示する。ゲノムの結果には、その種に複数のアセンブリがあるときにアセンブリ名を付ける。読み込んでいないデータの例（GRCh37 など）は表示しない。例は、代表的なものだけを表示し、残りは「more examples」を開くと出る（何を変換できるかを示すため、例自体は一通り用意する）。
...
- Results の見出しの横の **JSON** リンクは、いま画面に出している変換を同じ条件で `/v1/convert` に投げた結果を開く（スクリプトから呼ぶときの形そのもの。マウスを乗せると要求が見える）。
