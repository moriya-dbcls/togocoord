...
## 8. 保存先が増えたときの `/v1/meta`（2026-09-24）

保存先が51個になった時点で、応答に10.2秒かかっていた。UI は最初の変換の前にこれを待つ。原因はデータ量ではなく、各保存先に `liftover` の辺を少しだけ問い合わせていたこと。`edge` には `kind` の索引が無いため、アラインメントを持たない保存先では辺の表を最後まで走査していた（Ensembl の保存先だけで3.5秒）。構築時に記録した要約（`summary.edges`）を見て、その種類の辺を持つ保存先にだけ問い合わせるようにし、内容を変えずに10ミリ秒未満になった（crossings 20件、タグと生物種も同じ）。他の種類でも同じ問い合わせが必要になったら、取り込み時に `edge(kind)` の索引を作るのが筋だが、スキーマの版と作り直しが要る。
