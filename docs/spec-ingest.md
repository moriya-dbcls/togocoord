...
- **Chains are kept as they come**: unlike the PAF adapter (§16), the source side is not reduced to one-to-one, because UCSC's files are already netted. Where duplications leave one source position under several chains (1.7% of the covered positions of GRCh37 → GRCh38), all of them stay as separate edges; what the service returns from them is spec-service §2.
...
**Human T2T-CHM13v2.0 (2026-09-20)**: `chm13.sqlite` (the RefSeq annotation 2025-08 of `GCF_009914755.1`: GFF3, genome, RNA and proteins; the CDS self-validation leaves no unexplained mismatch, 36,846 carry an `/exception`), `chain_hs1ToHg38.sqlite` (13,185 chains, 856,771 blocks, 3,076,803 differing bases, 119 MB) and `chain_hg38ToHs1.sqlite` (7,679 chains, 823,316 blocks, 3,365,500 differing bases, 128 MB). Its UCSC name is `hs1` (added to `UCSC_DATABASES`). GRCh38 remains the hub for human, with CHM13 and GRCh37 as spokes (`hs1:chr7:142067515` → `NC_000007.14:140753336` on GRCh38 → `NC_000007.13:140453136` on GRCh37). CHM13 carries its own annotation, so proteins and UniProt are reached from it in one step, without the hub.
