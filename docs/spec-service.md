...
2. **同じ種のアセンブリは、経路が必要とすれば、指定がなくても1回またぐ。** 例: GRCh37 の位置 → chain → GRCh38 → CDS → タンパク質。経路には `liftover` の段が残るので、どこでアセンブリを移ったかが分かる。
...
- 非コード領域や配列の変わった遺伝子も変換するには、ゲノム全体のアライメント（T3）が要る。
