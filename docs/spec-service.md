...
変換先は「配列の種類」（`to`）と「範囲」（生物種 `taxon`、アセンブリ `assembly`）の2軸で指定する。

1. **既定では入力と同じ生物種・アセンブリに留まる。** 種やアセンブリをまたぐ段（liftOver の chain、別の種の同一配列）は、入力と違う `taxon` または `assembly` を指定したときだけ通る。同じ種を指定したときは、指定しないときと同じ。
2. **またぐのは1回まで、指定した範囲へだけ。** たとえば `taxon=10090` なら、マウスに入る段だけを通り、別の種に入る段は通らない。
3. **経路の途中で、今いる生物種とアセンブリを持ち回る。** 種をまたぐ段を通ると、その先の種に変わる。結果は持ち回った種で判定するので、配列そのものに taxon の記録がない配列（Ensembl など）も正しく扱える。配列の生物種は、その配列の記録、それを含む保存先の記録（`--taxon`）、同一配列が1種だけならその種、の順に決める。
4. **結果は指定した範囲のものだけ。** `assembly` は、ゲノムの結果だけを絞る（タンパク質や転写産物はアセンブリによらない）。
5. **範囲外の変換先（例: マウスを指定したときのヒトのタンパク質）は、そこから種をまたぐ段だけをたどる。** 同じ種の中でさらに広がらない。
6. **段数の上限**: 種をまたぐときは `CROSSING_HOPS`（2）を足す（またぐ段と、変換先の層に戻る段）。変換先を指定しないときは、またいだ先に着いたところで止める。結果は「入力が別の種で対応する位置」になる。
...
| メソッド | パス | 内容 |
|---|---|---|
| GET | `/v1/convert?loc=&to=&taxon=&assembly=&maxHops=&codon=never&tag=` | 変換。`taxon`（NCBI taxon の番号、`taxon:10090`、読み込んだ生物種の学名や一般名。例: `Mus musculus`、`mouse`）と `assembly`（例: `GRCm39`）で、変換先の範囲を指定する（§2.2）。知らない種やアセンブリは400。`to` は、種類（`genome` など）、名前空間（`uniprot` など）、配列（`refseq:NC_000001.11`）のいずれかで、複数指定できる。省略すると、直接つながる配列をすべて返す。`tag`（例: `MANE Select`）を指定すると、そのタグを持つ変換先だけを返す。結果には、変換先のタグ（`tags`）と生物種（`taxon`、`organism`）が付き、応答には入力の生物種（`inputTaxon`）が付く。UI は、入力と違う生物種の結果に生物種名を表示する |
| POST | `/v1/convert` | 一括変換。`{"locations": [...], "to": ..., "taxon": ..., "assembly": ..., "maxHops": ..., "codon": ...}`。最大1000件。個々の入力の誤りは、その要素に `error` として返す |
...
| GET | `/v1/meta` | 保存先ごとのファイル名、メタデータ（表示名、生物種、アセンブリ、元ファイル、構築日時）、内容の集計（spec-ingest §12）。`species` に、読み込んだ生物種（taxon、名前、アセンブリ）の一覧 |
...
`service/public/`（ビルド不要の HTML・JavaScript・CSS。コンセプト版の配色とフォントを引き継ぐ）。REST API だけを使う薄いクライアントで、状態は URL（`?loc=&to=&taxon=&assembly=&codon=`）に持つので、画面の URL をそのまま共有できる。

- Location ID を入力して、変換先（直接つながる配列、ゲノム、遺伝子領域、転写産物、タンパク質、構造）と生物種（既定は「same species」。読み込んだ生物種から選ぶ）を選ぶ。アセンブリの選択は、1つの種に複数のアセンブリを読み込んだときだけ表示する。コドン内の位置の表示を切り替えられる。
...
| 速さ（ゲノムへの変換とタンパク質への変換の合計、`taxon=10090`） | p50 15ms、p95 52ms、p99 100ms（ヒト側の範囲外の変換先からは、種をまたぐ段だけをたどる） |

遺伝子名が違う86件の多くは、オルソログの命名の違い（ZNF14 → Zfp709、CYP2F1 → Cyp2f2、REG1A → Reg1）や、多重遺伝子族（嗅覚受容体、苦味受容体、ヒストン）である。

**実例**: `uniprot:P07203:49`（ヒト GPX1）を `to=uniprot&taxon=10090` で変換すると、同一配列 → CDS → chain → CDS → 同一配列 の経路で `uniprot:P11352:47`（マウス Gpx1）になる（コスト4）。`refseq:NP_000572.2:49` → `refseq:NP_032186.2:47` も同じ。`refseq:NC_000075.7:106312500..106312550`（マウス9番染色体）→ `refseq:NC_000003.12:complement(join(51986735..51986765,51986769..51986774))`（途中の欠失は join になる）。

**残る課題**: オルソログの対応（Ensembl Compara など）は、種をまたぐ edge として §2.2 の規則で加えられる。chain のない種の組み合わせは、T3（GFA の持ち込み、自前の計算）で扱う。
