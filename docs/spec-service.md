...
| メソッド | パス | 内容 |
|---|---|---|
| GET | `/v1/convert?loc=&to=&maxHops=&codon=never&tag=` | 変換。`to` は、種類（`genome` など）、名前空間（`uniprot` など）、配列（`refseq:NC_000001.11`）のいずれかで、複数指定できる。省略すると、直接つながる配列をすべて返す。`tag`（例: `MANE Select`）を指定すると、そのタグを持つ変換先だけを返す。結果には、変換先のタグ（`tags`）と生物種（`taxon`、`organism`）が付き、応答には入力の生物種（`inputTaxon`）が付く。UI は、入力と違う生物種の結果に生物種名を表示する |
...
**検証**（`service/bench/verify-liftover.ts`、MANE Select のヒトのタンパク質から無作為に999残基）
...
| 速さ（ゲノムへの変換とタンパク質への変換の合計） | p50 11.8ms、p95 34ms、p99 53ms |

遺伝子名が違う86件の多くは、オルソログの命名の違い（ZNF14 → Zfp709、CYP2F1 → Cyp2f2、REG1A → Reg1）や、多重遺伝子族（嗅覚受容体、苦味受容体、ヒストン）である。

**実例**: `refseq:NP_000572.2:49`（ヒト）→ ゲノム → chain → マウスのゲノム → `refseq:NP_032186.2:47`（コスト4）。`refseq:NC_000075.7:106312500..106312550`（マウス9番染色体）→ `refseq:NC_000003.12:complement(join(51986735..51986765,51986769..51986774))`（途中の欠失は join になる）。

**残る課題**: オルソログの対応（Ensembl Compara など）を `orthology` の edge として加えるには、層の規則1の例外が要る。chain のない種の組み合わせは、T3（GFA の持ち込み、自前の計算）で扱う。
