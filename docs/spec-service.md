...
**Input without a database**: when an accession says by its syntax which database it belongs to, the namespace may be left out: `NP_000572.2:49` is read as `refseq:NP_000572.2:49`, `P07203` as `uniprot:P07203`. The candidate databases come from the ID syntax that [TogoID](https://togoid.dbcls.jp/) curates in its `config/dataset.yaml`, copied into `service/src/togoid-patterns.ts` by `scripts/togoid-patterns.ts` (no lookup at request time), together with the patterns of the namespace registry for what TogoID does not list (UniParc, PDB chains such as `4HHB.A`). Some ID forms belong to two databases — `P07203` is a UniProt accession and an INSDC one — and are decided by which sequence is loaded; when both or neither are, the request is a 400 listing the candidates (`candidates`), and the database has to be written. `/v1/location` returns the database it read in `written.namespace`.
...
`service/public/` (HTML, JavaScript, CSS and the favicon, with no build step; it keeps the colors and fonts of the concept version). A thin client that uses only the REST API. State is kept in the URL (`?loc=&to=&db=&taxon=&assembly=&codon=`), so the page URL can be shared as is.
...
- A **JSON** link beside the Results heading opens `/v1/convert` for the conversion on screen, with the same parameters (the link is what a script would call; mouse hover shows the request).
