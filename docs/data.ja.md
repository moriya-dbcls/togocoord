...
- すべてを作ると、取得済みのファイルからで、取り込みが合計約19分（8コアの Mac。保存先ごとのログの合計）。これに minimap2 のアライメント11本が加わる（ゼニゴケの 250Mb 同士で1本あたり約30秒）。取得は回線次第で、合計約7.6GB。
...
| | `human_rna` | RefSeq GRCh38.p14 の RNA の GenBank | 689MB |
| | `human_ensembl` | Ensembl release 116 の GFF3 とタンパク質配列 | 3.2GB |
| | `human_uniprot` | UniProt 参照プロテオーム UP000005640（current_release） | 58MB |
...
| | `chain_hg19ToHg38`、`chain_hg38ToHg19` | UCSC liftOver chain | 3.5MB、16MB |
...
| marchantia | `marchantia_v71` | INSDC MpTak_v7.1（GCA_039105155.1）の GenBank | 75MB |
| | `marchantia` | INSDC MpTak v3.1（GCA_003032435.1）の GenBank | 85MB |
| | `marchantia_uniprot` | UniProt UP000244005 | 7MB |
| | `mp_v31_to_v71`、`mp_v71_to_v31` | 両方のゲノム配列を minimap2 でアライメントした PAF（`work/` に作る） | 4.2MB、15MB |
| | `marchantia_tak2`、`marchantia_v51` | INSDC MpTak2_v7.1（GCA_037833965.1、Tak-2）、ASM993635v2（GCA_009936355.2、v5.1）の GenBank | 75MB、85MB |
| | `marchantia_cmv12` | cmMarPoly1.2（GCA_965642975.2）の assembly report とゲノム配列（注釈なし） | 0.6MB |
| | `marchantia_mpv4` | INSDC Mp_v4（GCA_001641455.1）の GenBank。UniProt の UP000077202 はこのアセンブリに基づく | 66MB |
| | `marchantia_uniprot_v4` | UniProt UP000077202（Mp_v4 のプロテオーム） | 7MB |
| | `mp_tak2_to_hub`、`mp_hub_to_tak2`、`mp_v51_to_hub`、`mp_hub_to_v51`、`mp_cmv12_to_hub`、`mp_hub_to_cmv12`、`mp_v4_to_hub`、`mp_hub_to_v4` | 中心（MpTak_v7.1）との minimap2 のアライメント（spec-ingest §20）。cmMarPoly1.2 だけ `asm20` | 17MB、17MB、1.6MB、2.4MB、12MB、10MB、10MB、12MB |
| sifts | `sifts` | SIFTS `uniprot_segments_observed.tsv.gz`（PDBe）、`pdb_seqres.txt.gz`（wwPDB）、上の UniProt すべて | 186MB |
...
**2026-09-18 の作り直しの確認**（保存先は当時29、現在51）: このスクリプトで全29の保存先を作り直し、以前の手作業の保存先と、配列・edge・注釈の数と自己検証の結果が一致することを確かめた。違いは chain の4つだけで、その後の改良による（両側のアセンブリの配列を記録する、パッチの UCSC 名を読み替えて取り込む chain が 5〜277 本増えた）。増えた chain のうち、RefSeq に収録されていない未配置 scaffold（`KI270752.1` など）に着くものは、RefSeq のゲノム配列で照合できないので自己検証が `skipped` になる。
...
**入れていないもの**: Col-CEN v1.2（セントロメアまでつながった Col-0 のアセンブリ。UCSC GenArk に TAIR10.1 との chain がある）は、INSDC の accession がなく GitHub（schatzlab/Col-CEN）でだけ配布されているので、配列の鍵を決められず、入れていない。MpTak1_v7.1（GCA_037833805.1）は、配列も CDS のタンパク質も中心（MpTak_v7.1）と同じ accession なので、入れても何も増えない（spec-ingest §20）。
