...
[English](spec-core.md) | 日本語

2026-09-18。[design.md](design.ja.md) の §3（Location ID）と §4（座標モデル）を、実装できる粒度まで厳密にしたもの。コアライブラリ（`core/`）はこの仕様に従い、テストデータ（`core/test/corpus/`）で検証する。
